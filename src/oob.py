...
def evaluate_RF_on_each_cancer_type(tcga_all_pred_df, type_df, tcga_tpm_df, tcga_snv, max_depth, max_features, max_samples, min_samples_split, min_samples_leaf):
    ...
    for i in tcga_cancer_types:
        ...
        clf = RandomForestClassifier(n_estimators=3000, max_depth=max_depth, max_features=max_features, max_samples=max_samples,
                                     min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, oob_score=True, n_jobs=40)
        clf.fit(X_i, y_i)
...
def make_abbv(type_df):
    ...
    cancer_type_abbv_df.abbv[cancer_type_abbv_df.type=='Cervical squamous cell carcinoma and endocervic...'] = 'CESC'
